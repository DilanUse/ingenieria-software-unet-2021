    ...
    @mouseout  = "mouseout">
      ...
      <div @click="clickGroup" class="group-header w-full">
        <span class="flex items-center w-full">
          ...
          <span
            ...
            {{ $tc(group.i18n, group.i18nCount || 2) || group.name }}
          </span>
          ...
          <vs-chip
            ...
            class="ml-auto mr-4"
            :color="group.tagColor">{{ group.tag }}</vs-chip>
        </span>
      ...
      <ul ref="items" :style="styleItems" class="vs-sidebar-group-items">
        <li v-for="(groupItem, index) in group.submenu" :key="index">
          ...
          <v-nav-menu-item
            v-else
            :icon-small="groupItem.icon === undefined || groupItem.icon === ''"
            :index  = "groupIndex + '.' + index"
            :to="groupItem.slug !== 'external' ? groupItem.url : null"
            :href="groupItem.slug === 'external' ? groupItem.url : null"
            :icon   = "itemIcon(groupIndex + '.' + index, groupItem.icon)"
            :slug   = "groupItem.slug"
            :target = "groupItem.target">
              <span class="truncate">
                {{ $tc(groupItem.i18n, groupItem.i18nCount || 2) || groupItem.name }}
              </span>
              <vs-chip
                v-if="groupItem.tag"
                class="ml-auto"
