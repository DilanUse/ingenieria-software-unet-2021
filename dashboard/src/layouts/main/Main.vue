...
export default {
  ...
  watch: {
    $route() {
      if (this.$route.name === 'contact-group-view' && this.pageTitle !== '') {
        this.routeTitle = this.pageTitle;
      } else {
        this.routeTitle = this.$route.meta.pageTitle;
      }
    },
    pageTitle(val) {
      if (this.$route.name === 'contact-group-view') {
        this.routeTitle = val;
      }
    },
    isThemeDark(val) {
  ...
  methods: {
    ...mapActions({
      connectAuthSocket: 'auth/connectAuthSocket',
  ...
  mounted() {
    this.connectAuthSocket({ vm: this });
  },
  beforeDestroy() {
